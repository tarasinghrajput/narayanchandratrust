      ...
      if (data.success) {
        const complaints = data.complaints.map((complaint) => {
          ...
          return {
            ...
            student: complaint.student?.name || "Unknown Name",  // ✅ Ensure student exists
            room: complaint.student?.room_no || "N/A",  // ✅ Prevents null errors
        ...
        setResolvedComplaints(complaints.filter((complaint) => complaint.status.toLowerCase() !== "pending"));
        setComplaints(complaints.filter((complaint) => complaint.status.toLowerCase() === "pending"));
      } else {
      ...
      <div className="flex md:gap-7 flex-wrap justify-center items-center gap-7">
        ...
        <div className="bg-neutral-950 px-10 py-5 rounded-xl shadow-xl w-96 max-h-64 overflow-auto">
          ...
          <ul role="list" className="divide-y divide-gray-700 text-white">
            {unsolvedComplaints.length === 0 ? "No new complaints!" : unsolvedComplaints.map((complaint) => (
              <li
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate text-white">{complaint.title || "No Title"}</p> {/* ✅ Handle missing title */}
                    <p className="text-sm truncate text-gray-400">{complaint.desc || "No Description"}</p> {/* ✅ Handle missing description */}
                  </div>
